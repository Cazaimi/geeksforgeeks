/* Connect Nodes at Same Level (Function Problem)
   ...
   if(p -> right == NULL and p -> left == NULL) {
        p -> nextRight = NULL;
          ...
          while(!q.empty()){
               ...
               if(current -> left != NULL) {q.push(current -> left);nextlevel++;}
               if(current -> right != NULL) {q.push(current -> right);nextlevel++;}
               ...
               if(i == thislevel){
                  ...
                  current -> nextRight = NULL;
                    ...
                    if(!q.empty()) current -> nextRight = q.front();
               }
          }
     }
}
